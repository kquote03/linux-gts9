/*
 * gts9-autofocus — contrast-detection autofocus for the Samsung Galaxy
 * Tab S9 rear camera. Ported from the SM-X710 gts9wifi-fedora fork; the
 * autofocus loop is untested on the X716B. Device nodes default to the
 * fork's (video0 / v4l-subdev30 sensor / v4l-subdev31 lens), which are
 * NOT stable -- override with GTS9_AF_VIDEO / GTS9_AF_SENSOR / GTS9_AF_LENS
 * (find them with `media-ctl -p`).
 *
 ...
 *
 * Build:
 *   gcc -O2 -o gts9-autofocus gts9-autofocus.c
 *
 * Use (the camera must be free — stop Snapshot and, if needed,
 * `systemctl --user restart wireplumber` first):
 *   ./gts9-autofocus            # find and apply best focus
 *   ./gts9-autofocus -p 512     # just set the lens position
 *   ./gts9-autofocus -g 64 -e 3260
 *
...
#define ENV_OR(name, def) (getenv(name) ? getenv(name) : (def))
#define DEV_VIDEO  ENV_OR("GTS9_AF_VIDEO", "/dev/video0")
#define DEV_LENS   ENV_OR("GTS9_AF_LENS", "/dev/v4l-subdev31")
#define DEV_SENSOR ENV_OR("GTS9_AF_SENSOR", "/dev/v4l-subdev30")
